...
if ( ! function_exists( 'newSeperator' ) ) :
function newSeperator($stringpassed) 
{
   $description = (string) $stringpassed;
			$description = str_replace("â€¢","<br />•",$description);
...
function WordLimiter($text,$limit=50){
...
function ccRecruit() {
	...
	$url = $_SERVER['PHP_SELF'].'?state=';
	//$applyURL = 'http://www.catercare.com.au/career/career/?jobid=';
	$applyURL = $_SERVER['PHP_SELF'].'?jobid=';
	...
	//var_dump($settings['plugin']['jobopenings']['jobs']);
	...
	//$apiRequestURL1 = 'https://recruit.zoho.com/ats/private/xml/JobOpenings/getRecords?apikey='.$api.'&ticket='.$ticket.'&sortColumnString=Date%20opened&sortOrderString=desc&fromIndex=1&toIndex='.$noOfRecords;
	
	
	$apiRequestURL = $xmlUrl;
	$xml = new Xml2Assoc();
		...
		<div class="header">
			...
			<span>
				...
				<select name="location" id="auslocation">
					...
					<option value="NewSouthWales" '.$NSW.'>New South Wales</option>
					<option value="Victoria" '.$VT.'>Victoria</option>
					<option value="SouthAustralia" '.$SA.'>South Australia</option>
					<option value="NorthernTerritory" '.$NT.'>Northern Territory</option>
					<option value="WesternAustralia" '.$WA.'>Western Australia</option>
				</select>
			</span>
		</div>';
	...
	if(!($_GET['jobid'])){
		foreach($settings['plugin']['jobopenings']['jobs'] as $job){
				foreach ($job['field'] as $field){
					
						 	if($field['label'][0]=='JOBOPENINGID'){
						 		$jobOpeningID = $field['description'][0];
						 	}
						 	if($field['label'][0]=='Date opened'){
						 		$openDate = $field['description'][0];
						 		//$openDate = strftime("%B %d",$openDate); 
						 	}
						 if($field['label'][0]=='Target date'){
						 		$targetDate = $field['description'][0];
						 	}
						 	if($field['label'][0]=='Posting title'){
						 		$positionName = $field['description'][0];
						 	}
						 	if($field['label'][0]=='Roles and responsibilities'){
						 		$roles = $field['description'][0];
						 	}
						 	if($field['label'][0]=='SEEK Description'){
						 		$seekdescription = $field['description'][0];
						 	}
						 	if($field['label'][0]=='Number of positions'){
						 		$noOfPositions = $field['description'][0];
						 	}
							if($field['label'][0]=='State'){
						 		$location = $field['description'][0];
						 	}
						 if($field['label'][0]=='Country'){
						 		$country = $field['description'][0];
						 	}
						 	if($field['label'][0]=='Work experience'){
						 		$workExperience = $field['description'][0];
						 	}
						 	if($field['label'][0]=='Required Skills'){
						 		$requiredSkills = $field['description'][0];
						 	} 	
						 	if($field['label'][0]=='Introduction'){
						 		$Introduction = $field['description'][0];
						 	}
						 if($field['label'][0]=='Application Summary'){
						 		$ApplicationSummary = $field['description'][0];
						 	} 
						if($field['label'][0]=='Published In'){
						 		$PublishedIn = $field['description'][0];
						 	}		
					 }
			...
			if($statename==$location){
				$isjob = $isjob+1;
					if($jobOpeningstatus!='Filled'){
						if($PublishedIn == 'Publish in Intranet only'){
						 ...
						 echo '<h4>'.newSeperator($workExperience).'</h4>';
						 echo '<b>Roles and Responsibilities</b><br />';
						 echo '<p>'.newSeperator(WordLimiter($seekdescription)).'</p>';
		...
		foreach($settings['plugin']['jobopenings']['jobs'] as $job){
				foreach ($job['field'] as $field){
					 ...
					 if($_GET['jobid']==$jobOpeningID){
						 ...
						 echo '<h3>'.$location.',&nbsp;'.$country.'</h3>';
						 echo '<h4>'.newSeperator($workExperience).'</h4>';
						 echo '<p>'.newSeperator($Introduction).'</p>';

						echo '<ul style="list-style:none">';
if(!$_GET['apply']){

						 echo '<li><b>Job Description</b><br />';
						 echo '<p>'.newSeperator($seekdescription).'</p></li>';
